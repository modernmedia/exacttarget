...
use ModernMedia\exactTarget\EtSoapClient;
use ModernMedia\exactTarget\EtSimpleOperators;
use ModernMedia\exactTarget\EtTriggeredSend;
use ModernMedia\exactTarget\EtTriggeredSendDefinition;
 ...
 *
 * @package exactTarget
 * @author  Micah Breedlove <ModernMedia@gmail.com> <micah.breedlove@blueshamrock.com>
 * @version 1.0
 */
class EtClient extends EtBaseClass
{
    ...
    public function __construct($username, $password, $serverInstance = '')
    {
        ...
        $this->log = new Monolog('exacttarget');
    }

    /**
     * Set Monolog handler
     * @param \Monolog\Handler\AbstractProcessingHandler $handler Monolog Handler instance
     */
        ...
        $objects   = array_map(
            function ($activeClass) use ($nsClass, $class, $className) {
                ...
                return new \SoapVar($activeClass,
                    SOAP_ENC_OBJECT,
                    $className,
                    \ModernMedia\exactTarget\EtClient::SOAPWSDL);
            },
            $activeClasses
        );

        $saveOption                    = new \ModernMedia\exactTarget\EtSaveOption();
        $saveOption->PropertyName      = $className;
        $saveOption->SaveAction        = constant("self::" . strtoupper($updateType));
        $requestOptions                = new \ModernMedia\exactTarget\EtCreateOptions();
        $requestOptions->SaveOptions[] = new \SoapVar($saveOption, SOAP_ENC_OBJECT, "SaveOption", self::SOAPWSDL);
        $request                       = new \ModernMedia\exactTarget\EtCreateRequest();
        $request->Options              = new \SoapVar($requestOptions, SOAP_ENC_OBJECT, "CreateOptions", self::SOAPWSDL);
